...
export default function PreviewPage() {
  ...
  const fetchFullContent = async () => {
    try {
      ...
      console.log(
        "Payment successful, storing content and redirecting...",
        data
      );
      ...
      try {
        ...
        if (contentData.length < maxUrlLength) {
          console.log(
            "Content small enough for URL, using URL parameter as tertiary backup"
          );
          // Use replace to avoid back button issues
          router.replace(`/article/${blobId}?content=${contentData}`);
        } else {
          console.log(
            "Content too large for URL, using content store and localStorage"
          );
          // Add a small delay to ensure content is properly stored
          setTimeout(() => {
            console.log(
              "Delayed redirect - content store size:",
              contentStore.getSize()
            );
            // Use replace to avoid back button issues
  ...
  useEffect(() => {
    const fetchENSName = async () => {
      if (!preview?.ownerAddress || !preview.ownerAddress.startsWith("0x"))
        return;

      setIsLoadingEns(true);
      try {
        const ens = await getENSName(preview.ownerAddress as `0x${string}`);
        setEnsName(ens);
      } catch (error) {
        console.error("Error fetching ENS name:", error);
      } finally {
  ...
  const getDisplayName = () => {
    ...
    return formatAddress(preview?.ownerAddress || "");
  };

  // Open explorer for the address
  const openExplorer = () => {
    if (!preview?.ownerAddress) return;
    const explorerUrl = `https://etherscan.io/address/${preview.ownerAddress}`;
